...
class Decoder(nn.Module):
    ...
    def __init__(self, atom_dim, hid_dim, n_layers, n_heads, pf_dim, decoder_layer=DecoderLayer, self_attention=SelfAttention,
                 positionwise_feedforward=PositionwiseFeedforward, dropout=0.1, device='cpu'):
        super().__init__()
        self.ln = nn.LayerNorm(hid_dim)
        ...
        self.do = nn.Dropout(dropout)
        self.fc_1 = nn.Linear(hid_dim, 128)
        self.fc_2 = nn.Linear(128, 2)
        #self.fc_3 = nn.Linear(128, 2)
        self.do_1 = nn.Dropout(0.2)
        # self.gn = nn.GroupNorm(8, 256)

    def target_mask(self, trg, trg_ptr):
        each_length = trg_ptr[1:]-trg_ptr[0:-1]
        new_trg = torch.zeros([len(each_length), each_length.max(), trg.shape[-1]]).to(trg.device)
        new_trg_mask = torch.zeros([len(each_length), each_length.max()]).to(trg.device)
        for index in range(len(trg_ptr)-1):
            new_trg[index, :each_length[index], :] = trg[:each_length[index],:]
            new_trg_mask[index, :each_length[index]] = 1
        return new_trg, new_trg_mask.type(torch.bool)

    def forward(self, trg, src, trg_ptr=None,src_mask=None):
        # trg = [batch_size, compound len, atom_dim]
        # src = [batch_size, protein len, hid_dim] # encoder output
        trg, trg_mask = self.target_mask(trg, trg_ptr)
        # trg = self.ft(trg)
        ...
        sum = torch.matmul(norm.unsqueeze(1), trg).squeeze(1)
